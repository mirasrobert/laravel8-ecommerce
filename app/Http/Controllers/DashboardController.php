...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $ordersToday = Order::select('transaction_no', 'created_at', 'deliveredAt')
            ->latest()
            ->whereDate('created_at', now(new DateTimeZone('Asia/Manila')))
            ->get()
            ->unique('transaction_no');

        $earningsThisMonth = Order::whereMonth('created_at', now(new DateTimeZone('Asia/Manila'))->month)->sum('amount');

        $products = Product::count();

        $users = User::whereHas('orders', function ($query) {
            $query->whereDate('created_at', now(new DateTimeZone('Asia/Manila')));
        })->get();
